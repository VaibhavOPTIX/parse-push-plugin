...
public class ParsePushPluginReceiver extends ParsePushBroadcastReceiver
{
    ...
    private static String getNotificationIconTag(Context context, JSONObject pnData) {
        return pnData.optString("icon", getAppName(context));
    }
    ...
    protected void onPushReceive(Context context, Intent intent) {
        if (ParsePushPlugin.isInForeground()) {
            ...
            if (parseMulti != null && !parseMulti.isEmpty() && parseMulti.equals("true")) {
                ...
                new NotificationBuilder(context, intent).execute();
    ...
    protected Bitmap getLargeIcon(Context context, Intent intent) {
        String name = getNotificationIconTag(context, getPushData(intent));
        URL url_value;
        Bitmap mIcon1 = null;
        try {
            ...
            if (url_value != null) {
                ...
                //BitmapFactory.decodeResource(context.getResources(),R.drawable.ic_dj);
            } else {
                return null;
            }
    ...
    private class NotificationBuilder extends AsyncTask<String, Void, Bitmap> {
        ...
        protected Bitmap doInBackground(String... URL) {
            return getLargeIcon(mContext,intent);
        }
        ...
        protected void onPostExecute(Bitmap result) {
            ...
            //
            JSONObject pnData = getPushData(intent);
            String pnTag = getNotificationTag(mContext, pnData);
            ...
            cIntent.putExtras(intent).setPackage(mContext.getPackageName());
            dIntent.putExtras(intent).setPackage(mContext.getPackageName());

            PendingIntent contentIntent = PendingIntent.getBroadcast(mContext, 0, cIntent, PendingIntent.FLAG_UPDATE_CURRENT);
            PendingIntent deleteIntent = PendingIntent.getBroadcast(mContext, 0, dIntent, PendingIntent.FLAG_UPDATE_CURRENT);
            ...
            int colorId = mContext.getResources().getIdentifier(RESOURCE_PUSH_ICON_COLOR, "color", mContext.getPackageName());
            if (colorId != 0) {
                builder.setColor(mContext.getResources().getColor(colorId));
            }

            NotificationManager notifManager = (NotificationManager) mContext.getSystemService(Context.NOTIFICATION_SERVICE);
            notifManager.notify(getNotificationTag(mContext, intent), 0, builder.build());
        }
    }
}
